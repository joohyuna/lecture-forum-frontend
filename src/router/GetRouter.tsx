...
import AdminDetailNoticePage from "../pages/admin/notice/detail/AdminDetailNoticePage.tsx";

// 회원의 권한에 따라 접근할 수 있는 주소를 판별하기 위해서
// react-router 라이브러리에서는 "로더(loader)"라는 기능을 제공함

// 라우팅 하기 전 로더가 미들웨어로 동작하여 접근 할 수 있는 유무를 판별

const adminLoader = () => {
    // zustand가 저장하고 있는 회원 정보에 접근해서 가져와야 함
    // 컴포넌트 안에서는 const {user, isLoggedIn} = useAuthStore();
    // 이 파일을 컴포넌트가 X , 화면에 그리기 전 단계의 처리 진행.
    // Next.js를 기준으로 설명하면, 지금여기는 Client Side가 아님
        ...
        children: [
            ...
            },
            {
                ...
                children: [
                    ...
                    { path: ":id", element: <AdminDetailNoticePage />},
                ],
            },
        ],
    },
