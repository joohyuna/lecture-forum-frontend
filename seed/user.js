...
// 환결설정 파일(.env)를 불러오기 위해 백엔드에서 썼던 것 처럼 dotenv라이브러리를 설치해서 사용할 것임
// 어디에서 쓸거야 javascript(Typescript X)에서 사용하기 위해 => @types/dotenv깔아주지 않아도 됨
// pnpm install dotenv
...
async function generateUser(count) {
    ...
    for (let i = 0; i < count; i++) {
        try {
            // fetch(전송해야하는 URL, 옵션)
            // 옵션은 객체이고, 그 안에 method, headers, body가 들어감
            ...
            // 그리고  Javascript에는 random이라는 메서드로 존재함
